...
Signal       | Description
---          |---
AxID/xID     | Transaction identifiers (ordering)
...
AxUSER/xUSER | User-defined (not recommended)
<!-- .element: style="font-size: 0.5em !important;" -->

> Where `x` is `W` for WRITES and `R` for READS.
<!-- .element: style="font-size: 0.4em !important;" -->
...
> It **must** be asserted in the source, but is **not required** in the destination (can be calculated based on AxLEN).
